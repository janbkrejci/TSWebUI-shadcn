...
| `pinnedColumnCount`        | `number`                  | `0`                    | Freeze this many leading columns to the left edge so they stay visible while the table scrolls horizontally. **Positional, not per-column**: it is whichever columns lead the CURRENT visible order, so reordering or hiding a column moves the frozen block with it. The selection / row-menu columns are frozen along with them          |
