...
import { ChevronsUpDown, Copy, Minus, Square, Target, X } from "lucide-react"
...
export const TsWindow = React.forwardRef<TsWindowRef, TsWindowProps>(
  (
      ...
      requestAnimationFrame(() => {
        if (contentRef.current && measureRef.current) {
          ...
          const totalContentHeight =
            innerHeight + paddingTop + paddingBottom + currentScrollbarHeight
          const headerHeight = 40
          const borderAdjustment = 2

          setSize((prev) => ({
            ...prev,
            height: totalContentHeight + headerHeight + borderAdjustment,
          }))
          setIsVisible(true)
        } else {
          setIsVisible(true)
    ...
    const handleResizeStop: RndResizeCallback = (_e, direction, ref, _delta, position) => {
      setInteracting(false)
      setGlobalUserSelect("")
    ...
    return (
      <Rnd
        ...
        className={cn(
          "flex flex-col overflow-hidden bg-background pointer-events-auto border rounded-lg dark:border-neutral-700",
          windowState === "maximized" ? "" : "shadow-xl"
        )}
      >
        {/* Header / Titlebar */}
        <div
        ...
        >
          <div className="flex items-center gap-2 overflow-hidden">
            <div className="flex gap-1.5 mr-2">
              ...
              <div
                className={cn(
                  "w-3 h-3 rounded-full bg-green-500 hover:bg-green-600 cursor-pointer flex items-center justify-center group",
                  windowState === "minimized" && "bg-gray-400 pointer-events-none"
                )}
                onClick={(e) => {
                  e.stopPropagation()
                  handleMaximize()
            ...
            <span className="text-sm font-medium truncate opacity-90">{title}</span>
          </div>
...
export const WindowProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <WindowContext.Provider
      value={{ openWindow, closeWindow, getWindow, windows, isInteracting, setInteracting }}
    >
      {children}
    </WindowContext.Provider>
  )
